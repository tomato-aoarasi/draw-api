...
public:
	...
	inline cv::Mat drawPlayerSingleInfoModernStyle(std::string_view song_id, Ubyte level,
		...
		DrawTool::transparentPaste(playerRKSBox, dstImg, 1907, 96);
		...
		// 上传日期
		{
			updateTimeSizeWidth = freetype2->getTextSize(updateTimeStr, 24, -1, nullptr).width;

			cv::Mat updateTimeBlank(cv::Size(updateTimeSizeWidth + 50, 33), CV_8UC4, cv::Scalar(0, 0, 0, 0));
			std::vector<std::vector<cv::Point>> contours{ std::vector<cv::Point>{
				cv::Point(0, updateTimeBlank.size().height),
				cv::Point(0 + 14, 0),//tan15.9 x 50 = 14.2428
		...
		DrawTool::transparentPaste(courseRating, dstImg, 1942, 49, 0.269f, 0.269f, cv::INTER_AREA);
		...
		// updateTime
		freetype2->putText(result, updateTimeStr, cv::Point(size_w - updateTimeSizeWidth - 27, size_h + 20 - 50), 24, cv::Scalar(255, 255, 255), -1, cv::LINE_AA, false);
		...
		// course
		freetype2->putText(result, std::move(PlayerCourse), cv::Point(font_offset_x, 78), 36, cv::Scalar(255, 255, 255), -1, cv::LINE_AA, true);
		...
		// rks
		freetype2->putText(result, std::move(player_rks_str), cv::Point(font_offset_x, 122), 32, cv::Scalar(0, 0, 0), -1, cv::LINE_AA, true);
		...
		freetype2->loadFontData("draw/phi/font/AdobeStdR.otf", 0);
		freetype2->putText(result, "Powerd by yuhao7370. Generated by tomato Team - Phigros.", cv::Point(16, 1059) + cv::Point(3, 4), 16, cv::Scalar(5, 5, 5), -1, cv::LINE_AA, false);
		freetype2->putText(result, "Powerd by yuhao7370. Generated by tomato Team - Phigros.", cv::Point(16, 1059), 16, cv::Scalar(255, 255, 255), -1, cv::LINE_AA, false);
		freetype2.release();
