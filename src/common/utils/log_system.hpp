...
private:
	...
	inline const static auto MS_LOGGER_NAME = Config::getConfig()["log"]["logger-name"].as<std::string>();
	inline const static auto MS_FILENAME = Config::getConfig()["log"]["filename"].as<std::string>();
	inline const static auto MS_HOUR = Config::getConfig()["log"]["hour"].as<int>();
	inline const static auto MS_MINUTE = Config::getConfig()["log"]["minute"].as<int>();
};
